...
pri = prices_[prices_$category %in% int,]
...
final = as.data.table(unsplit(x, f = as.factor(fin$listing)))
  
boxdata = data.table(name = character(), coeff = as.numeric(character()), std = as.numeric(character()))
boxdata2 = data.table(name = character(), coeff = as.numeric(character()), std = as.numeric(character()))
for (i in int) {
    
    mod = summary(lm(smooth_ahead_3 ~ rating, data = final[final$category == i,]))
    name = strsplit(categories_$category[i], "\\.")
    name = name[[1]][length(name[[1]])]
    boxdata = rbind(boxdata, list(name, mod$coefficients[2, 1], mod$coefficients[2, 2]))
    #pdat = pdata.frame(final[final$category == i,], index = c("vendor", "dat"))
    #mod = summary(plm(smooth ~ rating, data = final[final$category == i,], model = "within"))
    #boxdata2 = rbind(boxdata, list(name, mod$coefficients[1], mod$coefficients[2]))
}
...
par(mar=c(3, 7, 3, 3))
bxp(bxp.data, show.names = TRUE, horizontal = TRUE, las = 2)
title("Fig. 3: Correlation between sales and rating")
```

...
for (k in int) {
  ...
  pri = prices_[prices_$category == k,]
  ...
  fin$smooth = fin$smooth_ahead_3
  
  dat = pdata.frame(subset(fin, select = c("dat", "rating", "vendor", "smooth")), index = c("vendor", "dat"))
  
  mod = summary(plm(smooth ~ rating, data = dat, model = "within"))
  name = strsplit(categories_$category[k], "\\.")
  name = name[[1]][length(name[[1]])]

  boxdata = rbind(boxdata, list(name, mod$coefficients[1], mod$coefficients[2]))
...
par(mar=c(3, 7, 3, 3))
bxp(bxp.data, show.names = TRUE, horizontal = TRUE, las = 2)
title("Fig. 3: Correlation between sales and price")

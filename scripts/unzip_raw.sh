...
find -type f -exec sh -c 'temp=${0#./} && if [ ! -d "../raw/""${temp%.zip}" ]; then ../scripts/update_progress.sh "Found new zip file, ""$temp"", unzipping..." && unzip -q -o -d ../raw/"${temp%.zip}" "$0"; fi' {} \;;
...
while [ "`find . -type f -name '*.zip' | wc -l`" -gt 0 ]; do
	...
	find -type f -name "*.zip" -exec sh -c '../scripts/update_progress.sh "Unzipping ""$1" && unzip -q -o -d "${1%.*}" "$1" && rm "$1"' _ {} \;;
done
